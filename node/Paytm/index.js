...
app.post("/paynow", [parseUrl, parseJson], (req, res) => {
  ...
  if (
    ...
    /* where is app is hosted (render url)*/
      ...
      function (err, checksum) {
        ...
        var post_req = https.request(options, function (post_res) {
          ...
          post_res.on("end", function () {
            ...
            res.redirect(
              `http://localhost:4000/viewBooking?status=${_results.STATUS}&ORDERID=${_results.ORDERID}&date=${_results.TXNDATE}&bank=${_results.BANKNAME}`
            );
          });
        });
